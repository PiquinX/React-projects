...
export function ProductImages ({ imgs }) {
  ...
  return (
    <div className='flex h-full gap-5'>
        ...
        <div className='z-10 flex items-center w-full bg-transparent' >
            {
                imgs.length > 1
                &&
                <>
                <button onClick={handleDecrease} 
                    className={`${image === 0 ? 'cursor-default opacity-50' : 'hover:bg-black duration-300 hover:bg-opacity-5'} text-gray-600 w-12 h-full z-10 order-1`}>
                    <i className="fa-solid fa-chevron-left"></i>
                </button>
                <button onClick={handleIncrease} 
                    className={`${image === imgs.length - 1 ? 'cursor-default opacity-50' : 'hover:bg-black duration-300 hover:bg-opacity-5'} text-gray-600 w-12 h-full z-10 order-3`}>
            ...
            <div className='relative grid w-[85%] h-full overflow-hidden bg-white place-content-center order-2'>
                {
                    imgs.map((img, index) => {
