...
export function FavProductsController () {
  const { isLoading, favProducts, allFavProducts } = useFavProducts()

  if (isLoading) return <ProductsLoading />
  else if (!allFavProducts) return <p className='text-2xl text-center'>{"You don't have any items in your fav list."}</p>
  else if (!favProducts || favProducts.length === 0) return <ProductError />
  else return <Products products={favProducts} />
  // return (
